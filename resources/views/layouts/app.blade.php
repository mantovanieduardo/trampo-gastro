...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-gray-900 bg-gray-50">
        ...
        <div class="min-h-screen flex" x-data="{ sidebarOpen: false }">
            ...
            <!-- Sidebar -->
            <aside class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 flex flex-col transition-transform duration-300 ease-in-out lg:static lg:z-auto lg:translate-x-0"
                ...
                <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
                    ...
                    <a href="{{ route('vagas.index') }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                              {{ request()->routeIs('vagas.index') || request()->routeIs('vagas.show') ? 'bg-amber-500/10 text-amber-400 ring-1 ring-amber-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <a href="{{ route('notificacoes.index') }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                              {{ request()->routeIs('notificacoes.*') ? 'bg-amber-500/10 text-amber-400 ring-1 ring-amber-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                        <div class="relative flex-shrink-0">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
                            </svg>
                            @if($notifCount > 0)
                                <span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[9px] font-bold text-white">
                        ...
                        Notificações
                        @if($notifCount > 0)
                            <span class="ml-auto bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $notifCount }}</span>
                        @endif
                    </a>
                    ...
                    @if(Auth::user()->is_admin)
                        <div class="pt-5 pb-1.5 px-3">
                            <p class="text-[10px] font-semibold text-gray-600 uppercase tracking-widest">Administração</p>
                        </div>
                        <a href="{{ route('admin.dashboard') }}"
                           class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                                  {{ request()->routeIs('admin.*') ? 'bg-amber-500/10 text-amber-400 ring-1 ring-amber-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                @isset($header)
                    <header class="bg-white border-b border-gray-200 py-4 px-6 lg:px-8 sticky top-0 z-10">
                        <div class="flex items-center gap-3">
                            <!-- Hamburger mobile -->
                            <button @click="sidebarOpen = true"
                                    class="lg:hidden p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 flex-shrink-0">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            ...
                            <a href="{{ route('notificacoes.index') }}" class="relative p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
                                </svg>
                                @if($notifCount > 0)
                                    <span class="absolute top-1 right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[9px] font-bold text-white leading-none">
                                        {{ $notifCount > 9 ? '9+' : $notifCount }}
                                    </span>
                                @endif
                            </a>
